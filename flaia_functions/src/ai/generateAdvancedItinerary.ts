...
import { validateAdvancedItineraryRequest } from '../shared/validation';
...
export async function generateAdvancedItinerary(
    ...
    try {
        ...
        const validatedData = validateAdvancedItineraryRequest(data);
        ...
        const prompt = PromptBuilder.buildPersonalizedItineraryPrompt(
            validatedData,
            validatedData.questionnaire_answers,
            language,
            schemaVersion
        );
        ...
        console.log(`[${auth.uid}] [advanced_search] [${promptLogDocId || 'NO_LOG_ID'}] Advanced itinerary generated for ${validatedData.destination}, ${validatedData.number_of_days} days`);
        ...
        console.error('Generate Advanced Itinerary Error:', error);
        return handleAiError(error, 'generateAdvancedItinerary');
    }
}