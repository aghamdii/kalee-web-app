...
export const analyzeLabelImageFunction = onCall({
    region: 'europe-west1',
    timeoutSeconds: 120,
    memory: '1GiB',
    secrets: [apiKey],
    ...
    try {
        FoodUtils.logFunctionStart('analyzeLabelImage', userId, data);
        ...
        // Generate unified label analysis prompt
        const prompt = UnifiedPrompts.getLabelAnalysisPrompt(validLanguage, validUnitSystem, notes);
        ...
        try {
            ...
            // Generate content using structured output - Single comprehensive call for label analysis
            ...
            // Add warnings for low label readability
            if (analysisResult.foodValidity?.score < 0.5) {
            ...
            // Add warnings for poor image quality affecting OCR
            if (analysisResult.confidence < 0.6) {
                analysisResult.suggestions = [
                    ...(analysisResult.suggestions || []),
                    'For better accuracy, try taking the photo straight-on with good lighting and ensure the label is fully visible.'
                ];
            }
            
            // Validate nutrition label values are reasonable
            const nutrition = analysisResult.nutrition;
            if (nutrition) {
                // Check for unrealistic per-serving calories for packaged foods
                if (nutrition.calories > 1000) {
                    analysisResult.warnings = [
                        ...(analysisResult.warnings || []),
                        'Very high calories per serving detected. Please verify the serving size.'
                    ];
                }
                
                // Check calorie-macro alignment more strictly for labels (should be exact)
                ...
                if (caloriesDifference > 20) { // More than 20 calorie difference
                    analysisResult.suggestions = [
                        ...(analysisResult.suggestions || []),
                        'Macronutrient values may not perfectly align with stated calories. This could be due to rounding on the label.'
                    ];
                }
                
            }
            
            // Handle portion adjustments from user notes
            if (notes) {
                const lowerNotes = notes.toLowerCase();
                let multiplier = 1.0;
                
                // Parse common portion descriptions
                if (lowerNotes.includes('half') || lowerNotes.includes('1/2')) {
                    multiplier = 0.5;
                } else if (lowerNotes.includes('double') || lowerNotes.includes('2x') || lowerNotes.includes('two')) {
                    multiplier = 2.0;
                } else if (lowerNotes.includes('quarter') || lowerNotes.includes('1/4')) {
                    multiplier = 0.25;
                } else if (lowerNotes.includes('third') || lowerNotes.includes('1/3')) {
                    multiplier = 0.33;
                }
                
                // Apply multiplier if detected
                if (multiplier !== 1.0) {
                    if (analysisResult.nutrition) {
                        analysisResult.nutrition.calories *= multiplier;
                        analysisResult.nutrition.protein *= multiplier;
                        analysisResult.nutrition.carbs *= multiplier;
                        analysisResult.nutrition.fat *= multiplier;
                    }
                    
                    analysisResult.servingsAnalyzed = multiplier;
                    analysisResult.suggestions = [
                        ...(analysisResult.suggestions || []),
                        `Nutrition values adjusted for ${multiplier}x serving based on your notes.`
                    ];
                }
            }
            ...
            const promptLogDocId = await logFoodPromptAsync({
                user_id: userId,
                prompt_type: 'unified_label_analysis',
                ...
                food_metadata: {
                    ...
                    servings_analyzed: analysisResult.servingsAnalyzed || 1.0
                }
            });
            ...
            FoodUtils.logFunctionEnd('analyzeLabelImage', userId, true, duration);
            
            // Enhanced logging with correlation ID
            logger.info(`[${userId}] [unified_label] [${promptLogDocId || 'NO_LOG_ID'}] Analysis completed`, {
                sessionId,
                ...
                labelReadability: analysisResult.foodValidity?.score || 0,
                language: validLanguage,
                unitSystem: validUnitSystem,
                hasNotes: !!notes,
                servingSize: analysisResult.servingSize,
                servingsAnalyzed: analysisResult.servingsAnalyzed || 1.0,
            ...
            logger.error('Unified label analysis failed:', aiError);
            ...
            await logFoodErrorAsync({
                ...
                prompt_type: 'unified_label_analysis',
                session_id: sessionId,
                error_code: 'label_analysis_failed',
        ...
        const duration = timer.end();
        FoodUtils.logError('analyzeLabelImage', error, userId);
        FoodUtils.logFunctionEnd('analyzeLabelImage', userId, false, duration);
